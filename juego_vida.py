...
def guardar_tablero_a_csv(tablero, ruta_csv):
    with open(ruta_csv, 'w', encoding='utf-8') as archivo_csv:
        for fila in tablero:
            for j, celda in enumerate(fila):
                archivo_csv.write(str(celda))
                if j < len(fila) - 1:
                    archivo_csv.write(',')
            archivo_csv.write('\n')
...
class App:
    ...
    def guardar_resultado_final(self):
        nuevo_path = self.path_name[:-4] + '_final.csv'

        
        guardar_tablero_a_csv(self.tablero, nuevo_path)
        print(f"Simulación completada. Archivo guardado en: {nuevo_path}")
